      ...
      <div className="grid grid-cols-3 mb-16 gap-12 mx-56">
        {category.blogs.map((blog, index) => (
          <Link key={index} to={`/blog/${blog.id}`}>
            <article className="flex flex-col gap-6 rounded-lg bg-white">
              <div className="shrink-0 relative overflow-hidden">
                <img
                  className="blog-card-image w-full rounded-t-lg"
                  src="/big-blog.jpg"
                  alt="mwew"
                />
                <span className="absolute top-6 -left-5 bg-[#0077b5] text-white text-sm rounded-full py-2 pl-10 pr-6 transition-all duration-300 ease-in-out hover:-left-3">
                  {blog.mainCategory}
                </span>
              </div>
